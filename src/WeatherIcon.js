...
const WeatherIcon = ({ currentWeatherCode, moment }) => {
  const [currentIcon, setCurrentIcon] = useState("isClear");
  // // 當currentWeatherCode變化時,useMemo才會運行
  const theWeatherIcon = useMemo(() => weatherCode2Icon(Number(currentWeatherCode)), [currentWeatherCode]);
  useEffect(() => {
    setCurrentIcon(theWeatherIcon);
  }, [theWeatherIcon]);
  return <IconContainer>{weatherIcons[moment][currentIcon]}</IconContainer>;
};
