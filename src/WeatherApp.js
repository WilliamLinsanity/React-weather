...
const sunUrl =
  "https://opendata.cwb.gov.tw/api/v1/rest/datastore/A-B0062-001?Authorization=CWB-88D06302-5081-4A40-BD5D-2CCD4CDBEDF4";

const getMoment = (locationName) => {
  return fetch(sunUrl)
    .then((res) => res.json())
    .then((data) => {
      const location = data.records.locations.location.find((data) => data.locationName === locationName);
      ...
      const sunriseTimestamp = dayjs(`${todayDate.dataTime} ${todayDate.sunrise.parameterValue}`).unix();

      const sunsetTimestamp = dayjs(`${todayDate.dataTime} ${todayDate.sunset.parameterValue}`).unix();

      const nowTimeStamp = now.unix();

      return nowTimeStamp >= sunriseTimestamp && nowTimeStamp <= sunsetTimestamp ? "day" : "night";
    });
};
const WeatherApp = () => {
  // 找尋現在使用者所設定的城市
  ...
  useMemo(() => {
    const fetchingData = async () => {
      const [currentMoment] = await Promise.all([getMoment(currentLocation.cityName)]);
